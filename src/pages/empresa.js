...
export function renderEmpresa() {
      ...
      <form id="form-empresa">
        ...
        <div class="form-group" style="margin-bottom:24px">
          ...
          <div style="display:flex;gap:10px;flex-wrap:wrap">
            <label style="display:flex;align-items:center;gap:8px;cursor:pointer;padding:10px 18px;border:2px solid ${tipoDoc === 'cpf' ? 'var(--primary-400)' : 'var(--border-color)'};border-radius:var(--radius-md);background:${tipoDoc === 'cpf' ? 'rgba(99,102,241,0.08)' : 'transparent'};transition:all 0.2s">
            ...
            <label style="display:flex;align-items:center;gap:8px;cursor:pointer;padding:10px 18px;border:2px solid ${tipoDoc === 'cnpj' ? 'var(--primary-400)' : 'var(--border-color)'};border-radius:var(--radius-md);background:${tipoDoc === 'cnpj' ? 'rgba(99,102,241,0.08)' : 'transparent'};transition:all 0.2s">
    ...
    document.querySelectorAll('input[name="tipo-doc"]').forEach(radio => {
        radio.addEventListener('change', () => {
            ...
            document.querySelectorAll('label[style*="border"]').forEach(lbl => {
                const r = lbl.querySelector('input[type="radio"]');
                if (r) {
                    const active = r.checked;
                    lbl.style.borderColor = active ? 'var(--primary-400)' : 'var(--border-color)';
                    lbl.style.background = active ? 'rgba(99,102,241,0.08)' : 'transparent';
                }
            });
        });
    ...
    document.getElementById('save-empresa').addEventListener('click', () => {
        ...
        if (!codMunicipio || codMunicipio.length !== 7) {
            showToast('Código IBGE do Município deve ter 7 dígitos', 'error'); return;
        }
